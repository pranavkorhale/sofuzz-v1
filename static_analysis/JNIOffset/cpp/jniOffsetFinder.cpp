...
/*
Philipp Mao 9.2022:
This script looks for the offset of the target function in a native library
The code is taken from the harness with the goal of decoupling the harness from looking for the targetfunction offset. Also to get more representative statistics
The script writes a file to target_APK/app_name/signatures_libraries_offsets.txt this file contains the offsets and library name of the JNI function in the shared library
offset = &functionpointer - base_address
arguments: pathToApp LibraryName 
*/
...
std::vector<std::string> targetFunctionNames;
std::vector<std::string> targetSignatures;
std::vector<std::string> targetFunctionShortNames;
std::vector<std::string> targetFunctionClassNames;
...
int main(int argc, char *argv[]) {
		...
		/* read the signatures in signatures_pattern.txt into the lists */
		std::ifstream infile( targetAppPath + "/signatures_pattern.txt" );
		std::string sLineIn = "";
		while(std::getline(infile, sLineIn))
		{
			std::string functionName = sLineIn.substr(0,sLineIn.find(' '));
			// if the class is implemented in a subclass, replace the $ sign with _00024
			// Java_com_exmple_MainActivit$Test -> in native needs to be implmented as Java_com_exmple_MainActivity_00024Test
			functionName = std::regex_replace(functionName, std::regex("\\$"), "_00024");
			std::string functionSignature = sLineIn.substr(sLineIn.find(' ')+1, sLineIn.length());
			targetFunctionNames.push_back(functionName);
			targetSignatures.push_back(functionSignature);
			/* TODO: parsing the string like this is not always correct, to be clean/safe implement according to: https://docs.oracle.com/javase/6/docs/technotes/guides/jni/spec/design.html#wp615 */
			// extract java side native method name (last part of the Java_asdf_adf_functionName)
			std::string targetFunctionNameShort = std::regex_replace(functionName, std::regex("_1"), "#");
			targetFunctionNameShort = targetFunctionNameShort.substr(targetFunctionNameShort.rfind("_") + 1);
			// some crap to make sure that we extract the correct classname
			std::string tempFnameShort = std::regex_replace(targetFunctionNameShort, std::regex("#"), "_1");
			int targetFunctionShortLen = tempFnameShort.length();
			targetFunctionNameShort = std::regex_replace(targetFunctionNameShort, std::regex("#"), "_");
			targetFunctionShortNames.push_back(targetFunctionNameShort);
			// extract the classname to compare with the one sent with registerNatives
			std::string targetFunctionClassname = functionName.substr(5,-1);
			targetFunctionClassname = targetFunctionClassname.substr(0, targetFunctionClassname.length()-targetFunctionShortLen-1);
			targetFunctionClassname = std::regex_replace(targetFunctionClassname, std::regex("_"), ".");
			targetFunctionClassNames.push_back(targetFunctionClassname);
		}
        ...
        javaVM = javaVM_tmp;
        env = env_tmp;
		return findFunctionPtrSharedLib();
}
...
static auto load_art() -> std::pair<JavaVM *, JNIEnv *>
{
	...
	std::string jni_check = "-Xcheck:jni";
    JavaVMOption opt[] = {
    	{ apk_path.c_str(), nullptr},
        { lib_path.c_str(), nullptr},
		{ jni_check.c_str(), nullptr}
    };
...
/* 
Read /proc/self/maps to get maximum address of library
*/
long readMapsMax(){
	std::ifstream infile( "/proc/self/maps" );
	std::string sLineIn = "";
	...
	while(std::getline(infile, sLineIn))
    {
        if( sLineIn.find(targetLibName) != std::string::npos )
        {
            max =  std::stol( sLineIn.substr(sLineIn.find("-")+1, sLineIn.find(" ")), nullptr, 16 );
        }
    }
	return max;
}
...
/* Writes the found function offset to the outfile 
	Java_function_name ret:arg,arg, library offset
*/
void writeOffsetInfo(function_t *target, int index){
	std::string filepath = targetAppPath + "/signatures_libraries_offsets.txt";
	std::ofstream outputFile;
	outputFile.open(filepath, std::ios_base::app);
	long baseAddress = readMapsBase();
	long maxAddress = readMapsMax();
	if(baseAddress == -1){
		std::cout << "Failed to get base address !" << std::endl;
	}
	long funcLong = (long)target;
	long offset = funcLong - baseAddress;
	// sanity check the offset
	if(offset < 0 || funcLong > maxAddress || funcLong < baseAddress){
		return;
	}
	// for nested classes, reset _00024 back to $
	std::string targetFunction = targetFunctionNames[index];
	targetFunction = std::regex_replace(targetFunction, std::regex("_00024"), "$");
	std::string output = targetFunction + ' ' + targetSignatures[index] + ' ' + targetLibName + ' ' + std::to_string(offset) + '\n';
	std::cout << "Writing info to file: " << output << std::endl;
	outputFile << output;
	outputFile.close();
}
...
int findFunctionPtrSharedLib()
{
	...
	for(int i=0; i < targetFunctionNames.size(); i++)
	{
		targetFunctionPtr = (function_t *)dlsym(lib, targetFunctionNames[i].c_str());
		if (targetFunctionPtr)
		{
			printf("Found function pointer:%p\n", targetFunctionPtr);
			writeOffsetInfo(targetFunctionPtr, i);
		}
		targetFunctionPtr = NULL;
	}
	...
	//dlclose(lib);
	std::cerr << "no JNI_OnLoad exported by library " << targetLibName << std::endl;
    return 1;
}
...
jint RegisterNatives_fake(JNIEnv *penv, jclass clazz, const JNINativeMethod *methods, jint nMethods)
	{
    	...
    	jstring strObj = (jstring)env->CallObjectMethod(clazz, mid_getName);
    	const char* localName = env->GetStringUTFChars(strObj, 0);
    	std::string RNClassName = localName;
		std::cout << "	Class " << RNClassName << std::endl;
		...
		for (int i = 0; i < (int)nMethods; i++){
			...
			for (int k = 0; k < targetFunctionNames.size(); k++) {
				if (std::string(methods[i].name).compare(targetFunctionShortNames[k]) == 0 && std::string(targetFunctionClassNames[k]).compare(RNClassName) == 0) {
					targetFunctionPtr = (function_t *) methods[i].fnPtr;
					printf("Found function pointer:%p\n", targetFunctionPtr);
					writeOffsetInfo(targetFunctionPtr, k);
					targetFunctionPtr = NULL;
				}
			}
		}
		return 0;
		// call original RegisterNatives (no need)
