...
public class ExtraDataUseTransformerSp extends TargetedPathTransformerSp {
    ...
    public ExtraDataUseTransformerSp(String apkFilePath) {
        super(apkFilePath);
    }
    ...
    public boolean unitNeedsAnalysisTag(SootMethod method, String currClassName, Unit inUnit, Set<SootMethod> methodsWithSum) {
        if (Utils.isAndroidMethod(method)) {
            return false;
        }
        if (method.getName().startsWith("access$")) {
        ...
        if (inStmt.containsInvokeExpr()) {
            ...
            if (calleeMethod.hasTag("StringTag")) {
                /*
                if (methodsWithSum.contains(calleeMethod)) {
                    // only analyze from invocation if there is summary for it
                    return true;
                }
                 */
                return true;
