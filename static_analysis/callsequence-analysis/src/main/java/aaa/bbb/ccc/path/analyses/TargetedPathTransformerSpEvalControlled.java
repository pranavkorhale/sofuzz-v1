...
public class TargetedPathTransformerSpEvalControlled {
	...
	static Logger logger = LoggerFactory.getLogger(TargetedPathTransformerSpEvalControlled.class);
	...
	// all encompassing CatCounts
	public Map<String,Integer> intentCatCounts = new LinkedHashMap<String,Integer>();
	...
	public FileWriter outputPerApk = null;

	public TargetedPathTransformerSpEvalControlled(List jsons) {
		this.jsons = jsons;
	...
	public void main() throws IOException {
		...
		JSONObject fwobj = (JSONObject) jsons.get(2);
		JSONObject ic3exitsobj = (JSONObject) jsons.get(3);
		JSONObject faxobj = (JSONObject) jsons.get(4);
		JSONObject fwiobj = (JSONObject) jsons.get(5);
		JSONObject lbobj = (JSONObject) jsons.get(6);
		JSONObject aobj = (JSONObject) jsons.get(7);

		aobj.keySet().forEach(compKey ->
		{
			// per component
			String compName = (String) compKey;
			System.out.println("start: "+ compKey);
			JSONObject compContent = (JSONObject) aobj.get(compKey);

			List<String> manifestActions = null;
			List<String> manifestCategories = null;
			List<String> manifestData = null;

			// process component's intent filters if exists
			if (compContent.containsKey("IntentFilter")) {
				JSONObject filters = (JSONObject) compContent.get("IntentFilter");
				if (filters.containsKey("Action")) {
					JSONArray actionList = (JSONArray) filters.get("Action");
					manifestActions = new ArrayList<>();
					jsonarray2array(actionList, manifestActions);
				}
				if (filters.containsKey("Category")) {
					JSONArray categoryList = (JSONArray) filters.get("Category");
					manifestCategories = new ArrayList<>();
					jsonarray2array(categoryList, manifestCategories);
				}
				if (filters.containsKey("Data")) {
					JSONArray dataList = (JSONArray) filters.get("Data");
					manifestData = new ArrayList<>();
					jsonarray2array(dataList, manifestData);
				}
			}

			compContent.keySet().forEach(methodKey -> {
				// per method
				String methodSig = (String) methodKey;
				if (!methodSig.equals("IntentFilter")) {
					// ignore IntentFilter field
					JSONArray intents = (JSONArray) compContent.get(methodKey);
					Iterator intentsIter = intents.iterator();
					while (intentsIter.hasNext()) {
						JSONObject intent = (JSONObject) intentsIter.next();
						// get "lineNumber"
						JSONArray lineNumList = (JSONArray) intent.get("lineNumber");
						List<Long> lineNums = new ArrayList<>();
						jsonarray2array(lineNumList, lineNums);
						// get annotated attributes
						for (Object intentKeyRaw : intent.keySet()) {
							String intentKey = (String) intentKeyRaw;
							if (intentKey.equals("lineNumber")) {
								continue;
							}
							// TODO: intent attribute
			...
			System.out.println("done: "+ compKey);
		});
	...
	public void jsonarray2array(JSONArray jsonArray, List destArray) {
		Iterator arrayIter = jsonArray.iterator();
		while (arrayIter.hasNext()) {
			Object arrayItem = arrayIter.next();
			destArray.add(arrayItem);
		}
	}
...
//								return false;
	...
	private void doPathAnalysis(final SootMethod method, Collection<Future<?>> futures, boolean isIntra) throws IOException {
		...
		for (final Unit unit : units) {
			...
			if (attr != null) {
				...
				outputPerApk.write(this.apkName + "#" + attr + "#" + method.getDeclaringClass().getSuperclass().getName() + "#" + method.getDeclaringClass().getPackageName() + "#" + method.getSignature() + "#" + unit.getJavaSourceStartLineNumber() + System.getProperty("line.separator"));
